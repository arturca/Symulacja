...
FraktalT3D(n,r,phi,chi,xb,yb,zb)
...
d=0;
    for i=1:mN:mN^(n-1)
          z=2;
           for k=1:1:mN-1
                for j=z:1:n-1
                   line([A(j,i,1),A(j+1,i,1)],[A(j,i,2),A(j+1,i,2)],[A(j,i,3),A(j+1,i,3)],'Color',[0.5,0.8,0],'LineWidth',5);
                   d=d+1;
                 %pause(0.2);
...
% --------------- The end branches of tree ---------------
     ...
     for i=1:1:mN^n
        line([A(n,i,1),A(n+1,i,1)],[A(n,i,2),A(n+1,i,2)],[A(n,i,3),A(n+1,i,3)],'Color',[i/mN^n,0.75,sin(i/mN^n)],'LineWidth',30);
        if rand()<0.3&A(n,i,:)~=[1 1 1]
            fruit.strawberry([A(n,i,1),A(n,i,2),A(n,i,3)],0.1);
